...
export class FeedRepository {
  ...
  async findOne($filter: FilterQuery<FeedDocument>, $projection?: Projection<FeedDocument>) {
    ...
    return this.feedModel.aggregate([
      ...
      },
      {
        $lookup: {
          ...
          pipeline: [
            { $match: { $expr: { $eq: ["$_id", { $toObjectId: "$$shopId" }] } } },
          ],
          as: 'shop'
        }
      },
      ...
      },
      {
        $lookup: {
          ...
          pipeline: [
            { $match: { $expr: { $in: ["$_id", { $map: { input: "$$feedFileIds", as: "id", in: { $toObjectId: "$$id" } } } ] } } },
  ...
  async findOneFeedDetail(_id: ObjectId) {
    return this.feedModel.findOne({ _id, isDeleted: false })
      .populate({
        path: 'shopId',
  ...
  async findFeedLists(region: string, $skip: number, $limit: number): Promise<FeedDocument[]> {
    return this.feedModel.aggregate([
      {
